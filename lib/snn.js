...
class NeuralNetwork {
    constructor(layers, options) {
        ...
        if (options) {
            if (options.learning_rate) this.setLearningRate(parseFloat(options.learning_rate));
            if (options.activation && options.derivative) {
                if(options.activation instanceof Function && options.derivative instanceof Function) {
                    this.options.activation = options.activation;
                    this.options.derivative = options.derivative;
                } else {
                    console.error("Activation/derivative must be a function");
        ...
        for (let i = 1, j = 0; j < (this.layerCount); i++, j++) {
            if (layers[i] <= 0) {
                console.error("Layer must have at least one node (neuron).");
                return { layers: layers };
            }
